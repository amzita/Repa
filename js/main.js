...
function mostrarDescripcion() {
  ...
  let verHerramientasPasosBtn = document.querySelector(".button");
...
function mostrarHerramientasPasos(id, caja_descripcion_DOM) {
  ...
  <section class="cajota2">
    <h2 class="titulo_principal">${producto.titulo}</h2>
    <div class="caja_img">
      <img class="img" src="${producto.img}" />
    </div>
    <h2>Herramientas necesarias:</h2>
    <ul class="descripciones">
  ...
  let pasosHTML = `
    ...
    <ol class="descripciones">
      ${producto.pasos.map(paso => `<li>${paso}</li>`).join("")}
    </ol>
    <button class="btn">Click para ver el video </button>
    </section>
  `;

  caja_descripcion_DOM.innerHTML = herramientasHTML + pasosHTML;

  let verVideoBtn = caja_descripcion_DOM.querySelector(".btn");
